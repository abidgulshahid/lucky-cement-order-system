...
from .forms import CheckoutForm, CouponForm, RefundForm, CommentForm,ContactUsForm
from .models import (Item, Cart, OrderItem, Address, Comment, Payment, Coupon,
                     Refund, cash_on_delivery,Category, UserProfile, generate_code)
# 
...
class Contact_US(View):
    ...
    def post(self,request):
        ...
        if form.is_valid():
            form.save()
            messages.success(self.request, "We have recieved Your Message, Will Contact you Soon")
...
class SSLPayment(LoginRequiredMixin, View):
    def get(self, *args, **kwargs):
        ...
        cash_delivery = cash_on_delivery.objects.create(
            user=self.request.user,
            orders =order,
            no_of_items= no_of_items,
            amounts = amount,
            apartment_address = billing_address.apartment_address,
            street_address = billing_address.apartment_address
        )

        cash_delivery.save()
        if cash_delivery:
            tran_id = generate_code()
            today_date = date.today()
            expected_date =  str(today_date.day+7)+ "-"+str(today_date.month)+ "-"+str(today_date.year)
            messages.success(self.request, "Your Package will be Delivered in "+expected_date)
            return redirect('core:complete_payment', tran_id=tran_id, payment_type="SSL")
...
@login_required
def cod_complete_payment(request, tran_id, payment_type):
    order = Cart.objects.get(user=request.user, ordered=False)
    amount = int(order.get_total())
    payment = Payment()
    payment.user = request.user
